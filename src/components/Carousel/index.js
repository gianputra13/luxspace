...
export default function Caraousel({ children, refContainer }) {
  ...
  const onDragEnd = useCallback(
    (e) => {
      ...
      if (postFinal.current - posIntial.current < -threshold) {
        fnShiftItem(DIRECTION_LEFT);
      } else if (postFinal.current - posIntial.current > threshold) {
        fnShiftItem(DIRECTION_RIGHT);
      } else {
        refDragHandler.current.style.left = `${posIntial.current}px`;
      }
  ...
  const onDragStart = useCallback(
    (e) => {
      ...
      e.preventDefault();
      // console.log(e);
      posIntial.current = refDragHandler.current.offsetLeft;
      // console.log(posIntial.current);
